...
// ===== Configuration =====
// Custom Market Data Server URLs (Local)
// WebSocket endpoint for live market data
const CUSTOM_WS_URL = 'ws://localhost:8765';
// REST API endpoint for historical data and snapshots
...
async function connect(symbol: string): Promise<void> {
  ...
  pendingConnectTimer = setTimeout(() => {
    ...
    // Local server expects: ws://localhost:8765/stream?streams=symbol@depth@100ms/symbol@trade/symbol@miniTicker
    ...
    log('info', 'ws', 'ws.connecting', { symbol, url: wsUrl });
...
async function reconnectWithRetainedData(symbol: string): Promise<void> {
  ...
  pendingConnectTimer = setTimeout(() => {
    ...
    log('info', 'ws', 'ws.reconnecting', { symbol, url: wsUrl });
